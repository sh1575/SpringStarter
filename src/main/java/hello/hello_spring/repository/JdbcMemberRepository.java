...
public class JdbcMemberRepository implements MemberRepository {
    ...
    public Member save(Member member) {
        ...
        Connection conn = null; // DB와 연결하는 다리의 역할
        PreparedStatement pstmt = null; // SQL 쿼리문을 실행하는 객체
        ...
        try {
            ...
            throw new IllegalStateException(e); // 에러가 발생시 더 이상의 코드는 진행불가
        }finally{
            // 사용한 자원은 반드시 반납을 해줘야 함(open -> close)
            close(conn, pstmt, rs);
        }
    }
    ...
    public Optional<Member> findById(Long id) {
        ...
        try {
            ...
            if (rs.next()) { // 결과가 존재한다면
                ...
                // Optional 이용시 NullpointerException이 발생하지 않는다
                return Optional.of(member); // 결과를 Optional로 감싸서 반환
            } else {
                return Optional.empty(); // 결과가 없으면 Optional 반환
            }
        } catch (Exception e) {
    ...
    private Connection getConnection() {
        // 스프링 프렘임워크를 사용할 때 DataSourceUtils을 사용해야
        // 트랜잭션이 꼬이지 않고 Connection 객체를 얻어올수 있다.
        // 트랜잭션 - 쪼갤수 없는 논리적인 작업 단위
